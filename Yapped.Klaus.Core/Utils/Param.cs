...
// Taken From DS MAP STUDIO | Amazingly Laidout and created
namespace FSParam;
...
public class Param : SoulsFile<Param>
{
    ...
    public void ApplyParamdef(PARAMDEF def)
    {
        ...
        for (int i = 0; i < def.Fields.Count; i++)
        {
            ...
            if (!isBitType || (isBitType && field.BitSize == -1))
            {
                ...
                byteOffset += type switch
                {
                    PARAMDEF.DefType.s8 or PARAMDEF.DefType.u8 => 1,
                    PARAMDEF.DefType.s16 or PARAMDEF.DefType.u16 => 2,
                    PARAMDEF.DefType.s32 or PARAMDEF.DefType.u32 or PARAMDEF.DefType.f32 or PARAMDEF.DefType.b32 or PARAMDEF.DefType.angle32 => 4,
                    PARAMDEF.DefType.f64 => 8,
                    PARAMDEF.DefType.fixstr or PARAMDEF.DefType.dummy8 => (uint)field.ArrayLength,
                    PARAMDEF.DefType.fixstrW => (uint)field.ArrayLength * 2,
    ...
    protected override void Read(BinaryReaderEx br)
    {
        ...
        if (Rows.Count > 1)
            ...
            DetectedSize = (uint)((actualStringsOffset == 0 ? stringsOffset : actualStringsOffset) - Rows[0].DataIndex);
        else
            DetectedSize = 0;
